...
<head>
    ...
    <title>Invoices - My Bill Book</title>
    ...
    <style>
        ...
        @media (max-width: 576px) {
            .invoice-details { flex-direction: column; gap: 4px; }
                ...
                <div class="nav">
                    ...
                    <div class="collapse" id="collapseLayouts">
                        <nav class="sb-sidenav-menu-nested nav">
                            ...
                            <a class="nav-link" href="${pageContext.request.contextPath}/company/get-all-invoices">All Invoices</a>
                            ...
                            <a class="nav-link" href="${pageContext.request.contextPath}/company/export-to-pdf">Export Data</a>
                        </nav>
                    </div>
                </div>
            </div>
        ...
        <main class="container-fluid mt-4 px-3 px-md-4">
            ...
            <div class="row g-3 mb-4">
                <div class="col-sm-6 col-xl-3">
                    <div class="card shadow-sm border-0">
                        <div class="card-body text-center">
                            <div class="fs-6 text-muted">Total Invoices</div>
                ...
                <div class="col-sm-6 col-xl-3">
                    <div class="card shadow-sm border-0">
                        <div class="card-body text-center">
                            <div class="fs-6 text-muted">Paid</div>
                            <div class="fs-4 fw-bold text-success">${paidInvoices}</div>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6 col-xl-3">
                    <div class="card shadow-sm border-0">
                        <div class="card-body text-center">
                            <div class="fs-6 text-muted">Pending</div>
                            <div class="fs-4 fw-bold text-danger">${pendingInvoices}</div>
                        </div>
                    </div>
                </div>
                <div class="col-sm-6 col-xl-3">
                    <div class="card shadow-sm border-0">
                        <div class="card-body text-center">
                            <div class="fs-6 text-muted">Overdue</div>
                            <div class="fs-4 fw-bold text-warning">${overdueInvoices}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card mb-4">
                <div class="card-header d-flex flex-column flex-md-row justify-content-between gap-3">
                    <div>
                        <i class="fas fa-file-invoice me-2"></i><strong>All Invoices</strong>
                        <span class="badge bg-light text-dark ms-2">Total: ${totalInvoices}</span>
                    </div>
                    <div class="w-100 w-md-auto position-relative d-flex gap-2">
                        <input id="searchInput" class="form-control form-control-sm" placeholder="Search by Customer / Invoice No" autocomplete="off"/>
                            ...
                            <c:if test="${totalPages > 0}">
                                <c:forEach items="${invoices}" var="invoice">
                                    <tr>
                                        ...
                                        <td><a href="${pageContext.request.contextPath}/company/get-invoice/${invoice.custId}/${invoice.invoiceId}" class="btn btn-sm btn-link"><i class="fas fa-eye"></i> View</a></td>
                                    </tr>
                                </c:forEach>
                            </c:if>
                            </tbody>
...
<script>
    ...
    searchInput.addEventListener('input',function(){
        ...
        debounceTimer=setTimeout(()=>{
            fetch(contextPath+'/company/search-invoice?query='+query)
                .then(res=>res.json())
                .then(data=>{
                    searchDropdown.innerHTML='';
                    searchDropdown.style.display='block';
                    if(!data.length){
                        searchDropdown.innerHTML='<div class="list-group-item text-muted small"><i class="fas fa-file-invoice"></i> No matches found</div>';
                        return;
                    }
                    data.forEach(c=>{
    ...
    function resetSearch(){
        ...
        searchDropdown.innerHTML='';
        searchDropdown.style.display='none';
        window.location.href=contextPath+'/company/get-all-invoices';
    }
</script>
